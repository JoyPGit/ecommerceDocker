...
<br>
4. swagger <br>
url is working on adding dependency http://localhost:8080/swagger-ui.html <br>
configure controller
threw exception and it was caught
5. K8s <br>
6. redis <br>
7. kafka <br>
...
file upload
PostgreSQL is optimized for jsonb queries, so add jsonb in script

changes to add new table -> add flyway script
check strategy for id generation
if selecting sequence add both in script and a sequence generator for hibernate
in entity

This matches what Hibernate expects:

It lives under the commerce schema
It’s named user_documents_seq

IMP ensure that the sequence is generated by postgres, not hibernate, so
add schema in entity for  @SequenceGenerator
